...
## Intro
When a Relying Party (RP) receives a presentation, the EBW must verify the received attestations from the presentation. This document defines the common verification steps that an EBW MUST implement for all attestations independent from their type. 
...
The following diagram visualizes the chained certificates included in the header of each QEAA and each EAA:

![issuer_obligations.jpg](issuer_obligations.jpg)
...
- 4.2.2 Verification of the authenticity of the received  attestation (“genuinity” check)
- 4.2.3 Verification of the issuer’s identity
- 4.2.4 Verification that the issuer is authorized to issue the attestation and that the issuer is accepted according to verifiers policies


Attestation Holder related verification steps
...
Presenters EBW related  verification steps
- 4.2.7 Verification of the wallet integrity by verifying the wallet unit attestation
...
#### 4.2.4 Issuer related - authorization verification and issuer acceptance #### 

### Purpose
Verify that:
- the issuer is authorized to issue the specific type of attestation being presented (e.g., EBWOID, UBO, ...)
- this issuers attestation can be accepted based on verifiers internal policies.

Remarks:
- not every issuer will be trusted by an EBW owner even if he is authorized
- EBW owners internal policies consider also the EU and national regulation.

The authorization verification is based on the issuer identity data provided with the identity certificates/attestation included in the header of the attestation. 

### Questions to be answered
- Is the issuer authorized to issue the specific type of attestation?
- Can the verifier accept the issuer based on his internal policies? Does the verifier trusts the competence of this issuer?
...
#### 4.2.5 Holder related - attestation validity verification ####
#### Purpose
...
#### 4.2.6 Holder related - attestation revocation verification ####
### Purpose
...
This verification step is mandatory from Holder perspective if a Relying Party requests attestations with confidential data.

### Questions to be answered
- Is the wallet still valid or has it been revoked? Has the WUA been revoked?
- Are the wallet unit components authentic and valid?
- Does the holder has really received a request from another EBW relying party and not from another less trustfull software (e.g. EUDI Wallet Relying Party component)? 
Trust criteria: cryptographic possession trust, anti-cloning protection, key custody assurance, runtime integrity, execution trust 

The Wallet Unit Attestation also confirms to an EBW owner acting in the Holder role that he has received a request from another EBW and not for example from a Relying Party component used for example to communicate with EUDI wallets for natural persons. The WUA is part of the verifierInfo object included in each EBW RP request to enable mutual authentication (see ...link to mutual authentication) The EBW wallet is much more secure and prevent impersonation fraud much better than an unknown, uncertified Relying Party software without any conformity declaration. Therefore several EBW owner will deny any request for confidential data (e.g. UBO,...) received from Relying Party software.
...
### 4.2.8. Presenters wallet related - device binding ("copy or reply") verification ####
