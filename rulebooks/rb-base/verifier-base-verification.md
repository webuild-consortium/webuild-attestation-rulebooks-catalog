...
Both type of attestations therefore require the same verification steps. The differencies are described in corresponding paragraphs.

## 4.1 Issuer obligations ##

In this chapter mandatory rules for the metadata and the header content are defined.
- The issuer MUST include a public key generated by the holder EBW in the payload of each attestation. 
...
- the first certificate in the chain MUST be the EBWOID (or an equivalent certificate) In the header of the EBWOID (the EBWOID is technically a QEAA) according to the the above rules the x.509 certificate chain of the EBWOID provider are included.
...
During each EBW backend to EBW backend communication the first step is that both wallets perform the above steps at least for the attestations includedin the verifierInfo object provided by the EBW taht has initiated the communication (e.g. during a presentation request. The verifierInfo object includes at least the EBWOID and WUA attestations. That means that the above Relying Party obligation are also Holder EBW obligations. This aspect is described in ...link to mutual authentication.
