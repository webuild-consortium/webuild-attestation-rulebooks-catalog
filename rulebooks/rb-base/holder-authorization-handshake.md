...
4.2.8 RP EBWOID device binding ("copy or reply") verification. The checks answer the questions:
- Was the EBWOID issued and bound to this wallet unit? (Note: When the EBWOID was issued the RP EBW had to present a public key generated by his wallet to the EBWOID issuer and was identified by the issuer. This knowledge increases the confidence of the Holder, that the requesting EBW wallet was controlled by the EBW RP several time ago)
- Does the RP EBW controlls the private key during this transaction?

[Open topic related the use of a RP EBW component:
- Is it possible to issue RP EBW component bound attestations or x.509 certificates to a RP EBW component? If not the confidence of the Holder EBW will be reduced. The risk of impersonation of the requesting RP EBW component increases and therefore some Holder will not answer requests from RP EBW components.
- Does a RP EBW component supports copy and replay checks during attestation presentation?
]
Outcome of Phase 1:
...
sequenceDiagram
    autonumber
    box Holder
    participant HWF as Holder EBW Frontend
    participant HW as Holder EBW Backend
    end
    box Relying Party
    participant SW as RP EBW Backend
...
The decision will allways be based on the EBW owner internal policies that are based on EU and national regulations.

If the internal policy check is successful consent is granted, the Holder creates a Verifiable Presentations (VP Token) containing the EBWOID and the requested (Q)EAA(s) and presents them to the RP.

If the internal policy check is not successful consent is denied and the workflow terminates; no attestations are presented.

[Opent topics: 
- The mechanism to notify EBW owner employees is not defined
- The interface between the EBW backend and internal systems required during the consent decision is not defined
...
4.2.8 Holder EBWOID device binding ("copy or reply") verification to avoid impersonation. The checks answer the questions:
- Was the EBWOID issued and bound to the Holder EBW wallet unit? (Note: When the EBWOID was issued the Holder EBW had to present a public key generated by his wallet to the EBWOID issuer and was identified by the issuer. This knowledge increases the confidence of the RP, that the Holder EBW wallet was controlled by the EBW Holder several time ago)
- Does the Holder EBW controlls the private key during this transaction?
