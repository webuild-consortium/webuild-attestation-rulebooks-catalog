# Attestation Rulebook for attestations of type Micro-credential
...
| EW-DM-12-021 / EW-DM-12-022 | ARB_18 / ARB_19 | yes | 4.2 | Point e; recommendation for non-qualified EAA |
| EW-DM-12-023 / EW-DM-12-024 | ARB_20 / ARB_21 | yes | 4.2, 10 | `trust_anchor` metadata and the trust framework |
| EW-DM-12-029 | ARB_25 | yes | 4.1 | Legal category attribute; see the note recorded in section 4.1 |
...
**Legal category.** This Rulebook states in section 3.1 that the attribute
`attestation_legal_category` SHALL be included with the value `non-qualified-EAA`.

> **Open point for the quality assurance group.** Section 4.1 of the generic template
> states that `attestation_legal_category` is superseded by the `category` attribute of
> [ETSI TS 119 472-1] and SHALL NOT be used, and that a non-qualified EAA carries
> `category` with the value `eaa:eu:non-qualified`. Unlike PID, a migration target does
> therefore exist for this attestation. The attribute has been left as the authors
> wrote it, because renaming it would also change the normative claim tables and the
> worked examples in chapter 5, which is an authoring decision rather than a
> restructuring one. Note also that EW-DM-12-029 (legacy ARB_25) of ARF v3.0.0 is a
> SHALL that still requires `attestation_legal_category`, so the template and the ARF
> currently disagree.
...
The Micro-credential Attestation is issued using OpenID for Verifiable Credential
Issuance [OpenID4VCI], with the issuer metadata parameters recorded in section 3.1,
including `scope` `MicroCredential` and `format` `vc+sd-jwt`.

### 6.2 Presentation

The Micro-credential Attestation is presented using OpenID for Verifiable
Presentations [OpenID4VP]. Only remote presentation is in scope, consistent with the
formats recorded in chapter 5.
...
| Cryptographically bound to | attestation type or vct value, where used | EW-DM-12-032 |
| Relying Party must also verify a PID | no | EW-DM-12-031 |
| Binding subject | natural person | The learner to whom the learning achievement relates |
...
- The `exp` claim, where present, reflects the technical validity of the credential
  instance rather than the continued truth of the learning achievement, as recorded in
  chapter 8.
...
Remote presentation is in scope. Proximity presentation is not in scope in this
version, consistent with chapter 5, which records no mdoc encoding.

### 9.3 Transactional data

This Rulebook does not define transactional or payment-related data. Where a
Micro-credential Attestation is used in a transaction involving payment, such
requirements are out of scope of this version.
